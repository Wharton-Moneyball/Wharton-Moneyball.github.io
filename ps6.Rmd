...
Now that we've seen linear regression in R applied to MLB batting averages, let's practice what we've learned in a context we haven't worked with before. We will look at past results of the Ironman Triathlon and investigate relationships between racing splits and demographic variables. First let us download the data by going to our [course GitHub](https://github.com/Wharton-Moneyball/Wharton-Moneyball.github.io/tree/master/data) and saving the file titled 'Ironman Data.csv' to our Moneyball/data folder.
...
```{r create three plots, include = FALSE}
...
3. Now, we can determine if our quadratic model fits the data better than our linear model. To do this, we can compute the RMSEs of both models (as discussed in [Lecture 4](lecture4.html)) and see which is lower.
...
We can now clearly see that females have slower bike times than males. Note that the darker red line in the middle corresponds to our simple linear regression without the gender effect. 
