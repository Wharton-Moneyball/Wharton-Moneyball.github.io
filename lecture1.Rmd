...
**Throughout the rest of this course, we will be working almost exclusively with R scripts rather than directly entering commands into the console. We**
...
#remotes::install_github("jalapic/simpletable")
...
```{r multiline-syntax, include=TRUE}
nba_shooting_small <-
...
While it seems a bit cumbersome to write our code like this now, it will make our code much easier to read when we start to string multiple manipulations together using the pipe operator `%>%`, which we will meet in [Lecture 3](lecture3.html). 
