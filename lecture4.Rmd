...
In this format, we have a row for each player and two columns for his batting averages, one for 2014 and one for 2015. In order to convert our tbl into this format, we use the `pivot_wider` function. The `names_from` argument indicates which column will now become the column names instead of indicating values in the table - in this case `yearID`. The `values_from` argument indicates which columns' values will be compressed into a single column - in this case `BA`. We can use the `names_prefix` argument to easily rename the columns with `BA_` so that our columns don't start with numbers.
...
We can use this `batting_2014_2015_long` tbl to make visualizations, such as the one below, which shows the batting averages of each player in 2014 and 2015.
```{r}
